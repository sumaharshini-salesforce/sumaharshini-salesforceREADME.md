# 👋 Hi, I'm Sumaharshini Lanka

🚀 Salesforce Admin & Developer Fresher  
📍 Hyderabad, Bangalore | Open to Work  

---

⭐ Actively looking for Salesforce Admin / Developer opportunities in Hyderabad and Bangalore.

## 📫 Contact

📞  Contact Number: 9182196004
📧 Email: your-email@gmail.com  
🔗 LinkedIn: [Your LinkedIn URL](https://www.linkedin.com/in/yourprofile)  
📄 Resume: [Naukri Profile](https://www.naukri.com/mnjuser/profile)


## 💼 About Me

I am a passionate Salesforce Fresher with hands-on experience in both Administration and Development. I have practiced Apex Triggers, Record-Triggered Flows, and Admin tasks like Validation Rules and Reports & Dashboards.  

I am eager to contribute to Salesforce projects and deliver scalable CRM solutions.  

---

## 🔹 Technical Skills

**Salesforce Administration**
- User Management, Profiles & Permission Sets
- Object Relationships (Lookup & Master-Detail)
- Validation Rules & Formula Fields
- Reports & Dashboards
- Data Loader & Data Import Wizard

**Salesforce Development**
- Apex Programming & Triggers
- SOQL & SOSL
- Bulkification concepts
- Basic Test Classes

**Automation**
- Record-Triggered Flows
- Process Builder
- Approval Processes

---

## 🚀 Projects

1. **Account & Contact Automation** – Auto-create related records using Flows  
2. **Opportunity Amount Update Trigger** – Apex Trigger to update Account fields  
3. **Account Billing City Sync** – Trigger to sync Account Billing City with Contact Mailing City  

## 🎓 Education

- **Master of Computer Apllications (MCA)** in Computer Science  
  Vignan University, Guntur | 2025
  
- **Bachelor's Degree (B.sc)** in Chemistry
  Aadikavi Nanayya University | 2020

- **Higher Secondary (12th)**  
  VidyaVikas Junior College, Jangareddygudem | 2017 

- **Secondary School (10th)**  
  VidyaVikas HighSchool, Jangareddygudem | 2015
